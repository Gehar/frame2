...
//      mStr.set(nullptr, 0);
  ...
  template<size_t n_bits>
    ...
    Msg<n_bits>::ParseFromArray (const void* data)
    {
      ...
      /* Проверяем, что в буфере содержится хотя бы заголовок с длиной и поле crc */
  ...
  template<size_t n_bits>
    inline size_t
    Msg<n_bits>::ParseFromArray (const void* data, size_t size)
    {
      const Binary &bin =  *(reinterpret_cast<const Binary*> (data));
      /* Проверяем, что в буфере содержится хотя бы заголовок с длиной и поле crc,
       * а так же в буфере достаточно места для всего блока ЦИ */
      if ((bin.size < offsetof(Binary, str_byte0)) or (bin.size > size))
        return 0;
      mCrcError = bin.crc_error;
      size_t str_n = static_cast<uint8_t>(bin.size - offsetof(Binary, str_byte0));
      mStr.set(&bin.str_byte0, str_n);
