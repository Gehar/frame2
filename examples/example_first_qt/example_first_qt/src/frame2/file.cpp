 ...
 *
 *      Authors: A. Senchenko, A. Andreev
 *      Авторы: Андреев А. Г., Сенченко А. А.
 *      email: al-ndr@mail.ru, alsenchenko@mail.ru
 */
  ...
  if ((index == 0) or (index < current_pos_))
  {
    ...
    if (fseek(fp_, 0, SEEK_SET) != 0)
      throw std::runtime_error(
          "Error! Frame2::File.SetPos(): fseek(): " + std::string(strerror(errno)) + ", filename = \"" + path_ + "\", index = " + std::to_string(index));
