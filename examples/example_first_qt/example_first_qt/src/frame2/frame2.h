...
//    static const uint32_t scFrameId = 0x326d7266; /* = "frm2" */
    ...
    /* При смене версии изменить бинарный формат Auxiliary: если в текущий
     * класс добавить новое поле, то старые программы не смогут прочитать новый кадр */
  ...
  {
#ifdef DEBUG_FRAME2
    LOGMSG( "Размер переданного буфера %u", static_cast<unsigned int>(size));
    ...
    if (size < mSize)
    {
      LOGMSG("Размер буфера (= %d) меньше размера Frame2 (= %d)", static_cast<int>(size), static_cast<int>(mSize));
    ...
    {
      LOGMSG("Неверный идентификатор кадра: полученый кадр имеет ID = 0x%08x, ожидаемый идентификатор ID = 0x%08x",
             static_cast<unsigned int>(frame_id), static_cast<unsigned int>(scFrameId));
    ...
    {
      LOGMSG("Не поддерживаемая версия кадра: полученый кадр имеет V = %u, поддерживаемая версия V = %u",
             static_cast<unsigned int>(version), static_cast<unsigned int>(scVersion));
    ...
    while (expected_size < size)
    {
       ...
       *   3. Подкадр поддерживается, но имеет неверную длину (слишком большую
       *      или слишком маленькую). Читаем столько данных, сколько сможем
       *      оставшиеся пропускаем.
      ...
      switch (mSubframeId)
      {
        case GLO:
        {
          ...
          while (parsed_subframe_size < mSubframeSize)
          {
            ...
            {
              glo_len = add_glo()->ParseFromArray(buf.CurrentPointer(), mSubframeSize - parsed_subframe_size);
              buf += glo_len;
              parsed_subframe_size += glo_len;
            }
            if ((glo_size() >= mGlo.size()) or (glo_len == 0))
            {
              /* 3. Подкадр поддерживается, но имеет неверную длину (слишком большую или слишком маленькую) */
